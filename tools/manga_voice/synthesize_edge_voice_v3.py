...
class U:
    ...
    @property
    def slot(self) -> float:
        return max(.12, self.end - self.start - .085)
...
def load_utterances() -> list[U]:
    ...
    for node in tree.body:
        ...
        if value is not None:
            ...
            if len(result) != 47:
                raise RuntimeError(f'Expected 47 lines, found {len(result)}')
            return result
...
async def line_audio(u: U, voice: str, tag: str) -> tuple[AudioSegment, dict]:
    ...
    for _ in range(8):
        ...
        d = duration(src)
        attempts.append({'rate': rate, 'duration': round(d, 4)})
        if d <= u.slot:
            break
        rate += max(3, min(12, math.ceil((d / u.slot - 1) * 70)))
    if d > u.slot:
        raise RuntimeError(f'Line {u.index} does not fit: {d:.3f}s > {u.slot:.3f}s')

    # Keep the performance dry and untouched. Only remove sub-bass rumble and
    # place tiny edge fades to prevent clicks at edit points.
    subprocess.run([
        'ffmpeg', '-y', '-v', 'error', '-i', str(src),
        '-af', 'highpass=f=60,afade=t=in:st=0:d=0.012,areverse,afade=t=in:st=0:d=0.018,areverse',
    ...
    return seg, {
        **asdict(u),
        ...
        'rate_percent': rate,
        'pitch_hz': pitch,
        'rendered_duration': round(len(seg) / 1000, 4),
        'attempts': attempts,
    }
...
async def render(voice: str, count: int, tag: str) -> tuple[AudioSegment, list[dict]]:
    ...
    for u in UTTERANCES[:count]:
        ...
        master = master.overlay(seg, position=round(u.start * 1000))
        records.append(rec)
...
def master_audio(seg: AudioSegment, stem: str, seconds: float | None = None) -> tuple[Path, Path]:
    if seconds is not None:
        seg = seg[:round(seconds * 1000)]
    raw = OUT / f'_{stem}_raw.wav'
    wav = OUT / f'{stem}_48k.wav'
    mp3 = OUT / f'{stem}.mp3'
    seg.export(raw, format='wav', parameters=['-c:a', 'pcm_s24le'])
    subprocess.run([
        'ffmpeg', '-y', '-v', 'error', '-i', str(raw),
        '-af', 'loudnorm=I=-19:TP=-2:LRA=6',
        '-ar', str(SR), '-ac', '2', '-c:a', 'pcm_s24le', str(wav),
    ], check=True)
    subprocess.run([
...
async def main() -> None:
    ...
    manifest = {
        'engine': 'Microsoft Edge neural speech via edge-tts',
        'primary_voice': PRIMARY,
        'alternate_sample_voice': ALTERNATE,
        'sample_rate': SR,
        'duration': MASTER_MS / 1000,
        'utterance_count': 47,
        'audio_policy': {
            'dry_isolated_voice': True,
            'reverb': False,
            'echo': False,
            'delay': False,
            'room_ambience': False,
