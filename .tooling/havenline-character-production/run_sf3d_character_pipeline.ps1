...
$MinimumVramMiB = 6144
$RunnerTemp = if ($env:RUNNER_TEMP) { $env:RUNNER_TEMP } else { Join-Path $RepoRoot '.havenline-temp' }
...
$sourceMode = if ($Character -eq 'Character1') {
    'self-hosted-rtx-sf3d-recovery-after-trellis-rejection'
}
else {
    'self-hosted-rtx-sf3d'
}
...
$status = [ordered]@{
    ...
    approved = $false
    humanVisualReviewStatus = 'pending-blender-review'
    humanVisualApprovalRequired = $true
    unityIntegrated = $false
    failureReason = $null
}
if ($Character -eq 'Character1') {
    $status.recoveryReason = 'TRELLIS seed 9101 production artifact 8974534288 failed human four-view visual review.'
    $status.rejectedArtifactPolicy = '.tooling/havenline-character-production/character1-seed9101-visual-rejection.json'
}

function Save-Status {
    $path = Join-Path $OutputRoot 'sf3d-pipeline-status.json'
    $status | ConvertTo-Json -Depth 12 | Set-Content -Path $path -Encoding utf8
...
function Get-BlenderExecutable {
    $toolCacheRoot = if ($env:RUNNER_TOOL_CACHE) { $env:RUNNER_TOOL_CACHE } else { $RunnerTemp }
    $toolCache = Join-Path $toolCacheRoot 'havenline\blender-4.5.12'
    ...
    $archive = Join-Path $RunnerTemp 'blender-4.5.12-windows-x64.zip'
    Invoke-WebRequest `
...
try {
    ...
    if ($runtime.schemaVersion -ne 3) {
        throw "Unexpected runtime contract schema: $($runtime.schemaVersion)"
    }
    ...
    if ($Character -eq 'Character1') {
        $rejectionPath = Join-Path $ToolRoot 'character1-seed9101-visual-rejection.json'
        if (-not (Test-Path $rejectionPath)) {
            throw 'Character 1 visual rejection policy is missing.'
        }
        $rejection = Get-Content $rejectionPath -Raw | ConvertFrom-Json
        if ($rejection.humanVisualReview.status -ne 'rejected' -or
            $rejection.acceptedForUnityReview -ne $false -or
            $rejection.approved -ne $false -or
            $rejection.unityIntegrated -ne $false) {
            throw 'Character 1 recovery cannot run because the rejected source policy is invalid.'
        }
    }
    ...
    if (-not (Test-Path $reference)) {
        throw "Approved reference is missing: $reference"
    }
    ...
    if (-not $env:HF_HOME) { $env:HF_HOME = Join-Path $RunnerTemp 'havenline-huggingface' }
    if (-not $env:TORCH_EXTENSIONS_DIR) { $env:TORCH_EXTENSIONS_DIR = Join-Path $RunnerTemp 'havenline-torch-extensions' }
    ...
    $status.humanVisualReviewStatus = 'pending-blender-review'
    Save-Status
}
catch {
